...
//if the quantization coefficient is changed we have to recalculate the quantization matrix
//for coefficient <50 we have more compression and
//for coefficient >50 we have less compression
void new_Quantization(int **Q, int QC)
{
    ...
    for(int i=0; i<8; i++)
    {
        for(int j=0; j<8; j++)
        {
            Q[i][j]=(int) Q[i][j]*Q_C;//recalculating quantization matrix
...
//This function creates the BMP file header
...
//This will create BMP info header
//This contains all the information about image along with data
...
//using both the headers and our given image matrix it finally creates the bmp output image
...
//We take each of the individual 2D arrays R,G,B and apply DCT on them individually
    ...
    for(int xyz=0; xyz<3; xyz++)
    {
        ...
        int **Quantization;
//definition for normalized matrix (IMAGE->NORMALIZED IMAGE)
...
//definition for DCT coefficient matrix
...
//definition for our quantization matrix
...
//if quantization coefficient is changed we perform a check
//if necessary we create a new quantization matrix
        if(Quantization_coeff!=50)
        {
            new_Quantization(Quantization, Quantization_coeff);
        }
//storing the normalized image (since DCT works only in the range of -128 to 127 pixel values)
...
//Performing coefficient_matrix*normalized_image*coefficient_matrix_transpose
...
//Finally performing quantization on the obtained DCT coefficient matrix
//Dividing the final image by quantization matrix
        for(int i=0; i<HEIGHT_OF_IMAGE; i++)
        {
            for(int j=0; j<WIDTH_OF_IMAGE; j++)
            {
...
//Change the normalized image back to original by adding 128 to all the pixel values
        for(int i=0; i<HEIGHT_OF_IMAGE; i++)
        {
            for(int j=0; j<WIDTH_OF_IMAGE; j++)
            {
...
//Here we will take strides of 8 of DCT coefficient matrix and remove the rows and columns with 0's
    ...
    printf("\n Passes Completed \n");
//creating the final image from DCT matrix finally obtained
